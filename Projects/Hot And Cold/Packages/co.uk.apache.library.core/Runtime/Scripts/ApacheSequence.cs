...
namespace Apache.Core {
	...
	/// <summary>
	/// A sequence component maintains active delayed actions and sequences of actions, whether method invokes, coroutines, or tweens, 
	/// and provides a simple way to cancel them.
	/// </summary>
	public class ApacheSequence {
		...
		public delegate void CancelledEventHandler(CancelledStatus status);
		...
		private readonly HashSet<int> tweenIds = new HashSet<int>();
		...
		/// <summary>
		/// The MonoBehaviour this sequence is running on. Changing the MonoBehaviour will necessarily cause the sequence to cancel.
		/// </summary>
		public MonoBehaviour MonoBehaviour {
			...
			set {
				...
				// null out the game object reference so we can lazily re-fetch it from the MonoBehaviour when we need it.
				_gameObject = null;
			}
		}

		/// <summary>
		/// The game object which this sequence is running on.
		/// </summary>
		/// <remarks>
		/// The game object is retrieved from the <c>MonoBehaviour</c> lazily as access to a game object via a <c>Sequence</c> is a not-often-required feature.
		/// </remarks>
		...
		/// </summary>
		public bool IsActiveAndEnabled => monoBehaviour.isActiveAndEnabled;

		/// <summary>
		/// Do we have any active running delayed actions, queued actions, coroutines, or tweens?
		/// </summary>
		public bool IsRunning => actionQueue.Count > 0 || coroutines.Count > 0 || tweenIds.Count > 0;
		...
		/// <summary>
		/// Instantiates an <c>ApacheSequence</c> using the given <c>MonoBehaviour</c> to run coroutines.
		/// </summary>
		/// <param name="aMonoBehaviour">The <c>MonoBehaviour</c> on which to run coroutines.</param>
		public ApacheSequence(MonoBehaviour aMonoBehaviour) {
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence Do(float delay, Action action) {
		...
		/// <param name="action">The action to invoke.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence DoRealtime(float delay, Action action) {
			if (action == null) return this;
		...
		/// <param name="action">The action to invoke.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence Do(float delay, bool isRealtime, Action action) {
			return (isRealtime) ? DoRealtime(delay, action) : Do(delay, action);
		}
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence Coroutine(IEnumerator coroutine) {
		...
		/// <param name="action">The action to invoke.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence Queue(float delay, Action action) {
		...
		/// <param name="action">The action to invoke.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence QueueRealtime(float delay, Action action) {
			EnqueueAction(delay, action, true);
			return this;
		}

		/// <summary>
		/// Keeps a reference to a tween and handles cancellation later.
		/// </summary>
		/// <param name="tween">The tween to manage.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence Tween(LTDescr tween) {

			// N.B. Add() returns false if element already exists in set.
			tweenIds.Add(tween.id);

			// start a coroutine to remove the ID on tween completion.
			StartCachedCoroutine(
				 RemoveTweenIdOnComplete(tween.id, tweenIds)
			);
		...
		/// <summary>
		/// Keeps a reference to a tween and handles cancellation later, with the tween occurring in realtime uninfluenced by time scale.
		/// </summary>
		/// <param name="tween">The tween to manage.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence TweenRealtime(LTDescr tween) {
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence NextFrame(Action action) {
			if (action == null) return this;
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence AfterFrames(int numFrames, Action action) {
			if (action == null) return this;
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence EachFrame(Func<bool> func) {
		...
		/// <param name="action">The action to be performed in the determined frame.</param>
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence EndOfFrame(Action action) {
			if (action == null) return this;
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence EachEndOfFrame(Func<bool> func) {
			if (func == null) return this;
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence FixedUpdate(Action action) {
			if (action == null) return this;
		...
		/// <returns>This instance, useful for chaining.</returns>
		public ApacheSequence EachFixedUpdate(Func<bool> func) {
			if (func == null) return this;
		...
		public void Cancel() {
			...
			// cancel any tweens and empty the list.
		...
		/// <summary>
		/// Coroutine that processes the queue of actions, waiting the specified time for an action then invoking it, then continuing through the queue.
		/// </summary>
		/// <returns>Enumerator.</returns>
		...
		private void StartCachedCoroutine(IEnumerator enumerator) {
			...
			monoBehaviour.StartCoroutine(
				 StartCoroutineAndRemoveOnComplete(enumerator, coroutines)
			);
		}
		...
		/// <summary>
		/// Queues up an action to be invoked after all other queued actions are processed and the given delay (either time-scale-influenced or realtime).
		/// </summary>
		/// <param name="delay">The delay before invoking the action, after the existing queue has been processed.</param>
		/// <param name="action">The action to invoke.</param>
		/// <param name="isRealtime">Is the delay realtime or time scale influenced?</param>
		...
		private static IEnumerator StartCoroutineAndRemoveOnComplete(IEnumerator cachedCoroutine, HashSet<IEnumerator> removeFrom) {
			...
			removeFrom.Remove(cachedCoroutine);
		}

		private static IEnumerator RemoveTweenIdOnComplete(int id, HashSet<int> removeFrom) {

			// check every frame if the tween of the given ID is running.
			while (LeanTween.isTweening(id)) {
				yield return null;
			}

			// N.B. we don't need to worry about making it here when a tween is cancelled as opposed to naturally ended, because this
			// coroutine itself is cached and will be stopped when the sequence cancels.
