...
namespace Apache.Core {
	...
	[CustomEditor(typeof(Vector2Container))]
	public class Vector2ContainerEditor : ValueContainerEditor<Vector2> {
		...
		public new void OnEnable() {
			...
			initialValue = (Vector2)GetInitialValueFieldInfo().GetValue(valueContainer);
		}
		...
		public override void OnInspectorGUI() {
			...
			initialValue = EditorGuiLayoutUtils.Vector2FieldFullWidth(INITIAL_VALUE_LABEL, initialValue);

			// if the application is playing then draw the runtime values.
			if (Application.isPlaying) {
				valueContainer.Val = EditorGuiLayoutUtils.Vector2FieldFullWidth(RUNTIME_VALUE_LABEL, valueContainer.Val);
			}
			...
			Vector2 baseInitialValue = (Vector2)initialValueFieldInfo.GetValue(valueContainer);
