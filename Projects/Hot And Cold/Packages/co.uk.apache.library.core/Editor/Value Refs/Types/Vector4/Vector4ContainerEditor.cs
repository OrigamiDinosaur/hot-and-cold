...
namespace Apache.Core {
	...
	[CustomEditor(typeof(Vector4Container))]
	public class Vector4ContainerEditor : ValueContainerEditor<Vector4> {
		...
		public new void OnEnable() {
			...
			initialValue = (Vector4)GetInitialValueFieldInfo().GetValue(valueContainer);
		}
		...
		public override void OnInspectorGUI() {
			...
			initialValue = EditorGuiLayoutUtils.Vector4FieldFullWidth(INITIAL_VALUE_LABEL, initialValue);

			// if the application is playing then draw the runtime values.
			if (Application.isPlaying) {
				valueContainer.Val = EditorGuiLayoutUtils.Vector4FieldFullWidth(RUNTIME_VALUE_LABEL, valueContainer.Val);
			}
			...
			Vector4 baseInitialValue = (Vector4)initialValueFieldInfo.GetValue(valueContainer);
